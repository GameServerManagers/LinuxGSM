...
# Metamod:Source 2
mmsource2version="2.0"
mmsource2scrapeurl="https://mms.alliedmods.net/mmsdrop/${mmsource2version}/mmsource-latest-linux"
mmsource2latestfile=$(wget "${mmsource2scrapeurl}" -q -O -)
mmsource2downloadurl="https://www.metamodsource.net/latest.php?os=linux&version=${mmsource2version}"
mmsource2url="${mmsource2downloadurl}"
...
weaponpaintslastbuild=$(curl --connect-timeout 10 -sL https://api.github.com/repos/Nereziel/cs2-WeaponPaints/releases/latest | jq '.assets[] | select(.name == "WeaponPaints.zip")')
weaponpaintslatestfile=$(echo "${weaponpaintslastbuild}" | jq -r '.name')
weaponpaintslatestlink=$(echo "${weaponpaintslastbuild}" | jq -r '.browser_download_url')
...
# CS2 Mods
mod_info_mmsource2=(MOD "mmsource2" "Metamod: Source 2 (dev build)" "${mmsource2url}" "${mmsource2latestfile}" "0" "LowercaseOff" "${systemdir}" "addons/metamod/metaplugins.ini;" "source2;" "GAMES" "NOTGAMES" "https://www.metamodsource.net" "Plugins Framework ported to Source 2")
