...
fn_menu_string() {
	...
	case "$(basename "${menucmd}")" in
		whiptail|dialog)
			fn_menu_whiptail_string "${menucmd}" selection "${title}" "${caption}" "${options}" 40 80 30 "${prefix}";;
		*)
			fn_menu_bash_string selection "${title}" "${caption}" "${options}";;
	esac
