...
fn_monitor_check_lockfile(){
	...
	if [ ! -f "${rootdir}/${selfname}.lock" ]; then
	...
	if [[ "$(cat "${rootdir}/${selfname}.lock")" =~ [A-Za-z] ]]; then
			date '+%s' > "${rootdir}/${selfname}.lock"
	fi
}
...
for queryattempt in {1..5}; do
	...
	fn_sleep_time
	# querydelay
	if [ "$(cat "${rootdir}/${selfname}.lock")" -gt "$(date "+%s" -d "${querydelay} mins ago")" ]; then
