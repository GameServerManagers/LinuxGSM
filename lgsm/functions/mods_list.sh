...
#sm-ripext
ripextscrapepath="https://api.github.com/repos/ErikMinekus/sm-ripext/releases/latest"
ripextlatestversion="$(curl --silent ${ripextscrapepath} | jq -r .tag_name)"
ripextlatestfile="sm-ripext-${ripextlatestversion}-linux.tar.gz"
ripextdownloadurl="https://github.com/ErikMinekus/sm-ripext/releases/latest/download/${ripextlatestfile}"
# Oxide
...
mod_info_ripext=( MOD "ripext" "sm-ripext" "${ripextdownloadurl}" "${ripextlatestfile}" "0" "LowercaseOff" "${systemdir}" "OVERWRITE" "ENGINES" "Counter-Strike: Global Offensive;" "NOTGAMES" "https://github.com/ErikMinekus/sm-ripext" "SourceMod REST in Pawn Extension" )
...
mods_global_array=( "${mod_info_metamod[@]}" "${mod_info_sourcemod[@]}" "${mod_info_steamworks[@]}" "${mod_info_gokz[@]}" "${mod_info_ttt[@]}" "${mod_info_get5[@]}" "${mod_info_pug[@]}" "${mod_info_ripext[@]}" "${mod_info_ulib[@]}" "${mod_info_ulx[@]}" "${mod_info_utime[@]}" "${mod_info_uclip[@]}" "${mod_info_acf[@]}" "${mod_info_acf_missiles[@]}" "${mod_info_acf_sweps[@]}" "${mod_info_advdupe2[@]}" "${mod_info_pac3[@]}" "${mod_info_wiremod[@]}" "${mod_info_wiremodextras[@]}" "${mod_info_darkrp[@]}" "${mod_info_darkrpmodification[@]}" "${mod_info_rustoxide[@]}" "${mod_info_hwoxide[@]}" "${mod_info_sdtdoxide[@]}" )
