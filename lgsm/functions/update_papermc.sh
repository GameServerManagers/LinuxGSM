...
# Description: Handles updating of PaperMC servers.
...
fn_update_papermc_dl(){
	fn_fetch_file "https://${remotelocation}/api/v1/paper/${paper_version}/latest/download" "${tmpdir}" "paper.${remotebuild}.jar"
	echo -e "copying to ${serverfiles}...\c"
	cp "${tmpdir}/paper.${remotebuild}.jar" "${serverfiles}/paper.jar"
	local exitcode=$?
	if [ "${exitcode}" == "0" ]; then
		fn_print_ok_eol_nl
		fn_script_log_pass "Copying to ${serverfiles}"
		chmod u+x "${serverfiles}/paper.jar"
		echo "${remotebuild}" > "${configdirserver}/paper_localbuild.cfg"
		fn_clear_tmp
	else
...
fn_update_papermc_localbuild(){
	...
	if [ ! -f "${configdirserver}/paper_localbuild.cfg" ]; then
		...
		localbuild=$(head -n 1 "${configdirserver}/paper_localbuild.cfg")
	fi
...
fn_update_papermc_remotebuild(){
	...
	remotebuild=$(${curlpath} -s "https://${remotelocation}/api/v1/paper/${paper_version}/latest" | jq -r '.build')
	if [ "${installer}" != "1" ]; then
